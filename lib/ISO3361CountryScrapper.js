...
module.exports =
	...
	class ISO3361RegionScrapper {
		...
		static async scrapRemoteData() {
			...
			console.log(`Scrapping ISO-3361 data from ${DOMAIN_UNECE + SUBDIVISIONS_URN}`)
			const source = await fetch(DOMAIN_UNECE + SUBDIVISIONS_URN)
			...
			return allPromisesProgress(
				'Scrapping country details (ISO-3361):',
				countries.map(fetchCountryDetails)
			)
		}
	}
...
async function fetchCountryDetails(country) {
	...
	if (country.hasLink) {
		const $ = cheerio.load(
			await fetch(DOMAIN_UNECE + country.link)
		)
